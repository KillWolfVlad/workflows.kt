    ...
    describe("init") {
        describe("when lock is not set") {
            ...
            it("must set lock") {
                ...
                fieldValues shouldBe mapOf(defaultWorkflowConfig.workerId to "OK")
            }

            it("must set ttl") {
                val ttl = redisClient.hPTTL(WORKFLOW_WORKERS_KEY, defaultWorkflowConfig.workerId).first()

                ttl shouldBeInRange 0L..defaultWorkflowConfig.lockTimeout.inWholeMilliseconds
            }
        }

        describe("when lock already set") {
            beforeEach {
                redisClient.hSet(WORKFLOW_WORKERS_KEY, defaultWorkflowConfig.workerId to "OK")
